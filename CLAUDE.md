...
`src/lib/plan-link.ts` يحلّها: يبحث عن الاكتفيتي المخطط لنفس السوق والبراند
والنوع في **الشهر الحالي ثم الشهر السابق فقط**، ويأخذ شهره. ما لقاه →
`activity_id = null` والشهر شهر الإرسال، ويُعرض **«خارج الخطة»**.

الموظف يبقى حرّاً يختار — الربط **يُكتشف ولا يُفرض**، لأن استاندات تصل خارج
الخطة فعلاً وقفل الشاشة عليها يعني إهمالها لا منعها.
